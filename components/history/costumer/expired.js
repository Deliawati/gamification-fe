...
export default function Expired() {
  const [costumerExpired, setCostumerExpired] = useState();
  const { dataRewardCostumersExpired } = useSelector(costumerState);
  useEffect(() => {
    if (dataRewardCostumersExpired) {
      const newData = Object.values(dataRewardCostumersExpired);
      setCostumerExpired(newData);
    }
  }, [dataRewardCostumersExpired]);
  return (
    <div className="flex flex-col gap-6 h-screen">
      {costumerExpired?.map((item) => (
